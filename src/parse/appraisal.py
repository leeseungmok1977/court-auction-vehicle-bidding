...
def parse_appraisal(text: str, today: Optional[date] = None) -> Optional[dict]:
    ...
    # ⚠ "시동의 작동여부는 확인이 불가"는 **못 봤다**는 뜻이지 못 움직인다는 뜻이 아니다.
    #    코퍼스 실측(1,325건 중 '시동' 언급 298건): 과탐 후보 27건이 이 형태였다.
    #    반대로 "시동상태는 불량"·"시동이 안되는 상태"는 미탐 9건이었다(4회차 중고차 지적).
    if re.search(r"(시동|운행)[^.]{0,16}(확인\s*(이|은)?\s*불가|확인\s*되지\s*않|미확인)", clean):
        runnable = None                      # 확인 불가 — '불가'로 세지 않는다
    elif re.search(r"(시동|운행)[^.]{0,16}(불가|불능|불량|안\s?됨|안\s?되|(?:되|걸리|켜지)지\s*않)", clean):
        runnable = False
