...
- ★ **차량 사진의 번호판·차대번호(VIN) 노출은 결함이 아니다 — 결함으로 올리지 마라.**
  법원이 로그인 없이 공개하는 원본을 그대로 싣는 것이 **대표 결정(2026-09-23)이고
  재론하지 않는다**(`docs/compliance-review.md` §7). 이용자가 원본 그대로를 요구한다.
  노출 수치는 그 절에 이미 실측으로 적혀 있다 — 다시 세어 '오너 승인 필요'로 올리지 마라.
