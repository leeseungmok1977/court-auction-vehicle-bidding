...
def test_전제_가격대_층과_다른_층의_값이_다르다():
    ...
    assert rows[("가격대", "2,000만 이상")]["mae"] == 12.0
...
def test_층_경계는_기존_값_그대로다():
    ...
    ★ 2026-09-23: 이 단언이 **오기를 고정하고 있었다.** 마지막 층은 `>= 20_000_000` 인데
      라벨이 `2,000만 초과`라, 시세가 정확히 2,000만원인 물건에 "초과"라고 찍혔다.
      경계 **값**은 그대로 두고(아래 숫자 불변) 낱말만 `이상`으로 고친다 — 테스트가
      "그게 맞다"며 다음 사람을 막지 않도록 같은 커밋에서 단언도 바꾼다.
    """
    for med, label in [(4_999_999, "500만 이하"), (5_000_000, "500~1,000만"),
                       (9_999_999, "500~1,000만"), (10_000_000, "1,000~2,000만"),
                       (19_999_999, "1,000~2,000만"), (20_000_000, "2,000만 이상"),
                       (20_000_001, "2,000만 이상")]:
        assert service._price_band(med) == label, med
    # 경계값이 라벨과 모순되지 않는가 — 낱말이 다시 '초과'로 돌아가면 여기서 걸린다.
    assert "초과" not in (service._price_band(20_000_000) or ""), (
        "시세가 정확히 2,000만원인 물건이 이 층에 들어오므로 '초과'는 거짓이다")
