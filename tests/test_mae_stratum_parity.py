...
★ 반드시 지킬 예외: **§모델 검증 실적 타일(`report.html:1075`)은 전체평균이 맞다.**
  그 섹션은 이 물건이 아니라 모델 전체를 말한다("예측이 실제 낙찰을 얼마나 맞혔는가").
...
# 모델 전체를 말하는 타일은 숫자가 **앞**에 온다: "±8.9% 예측 오차".
_MODEL_WIDE = re.compile(r"±\s*([\d.]+)\s*%\s*예측\s*오차")
...
def test_모델_검증_실적_타일은_전체평균이_맞다(client):
    """★ 예외 조항. 이 섹션은 이 물건이 아니라 **모델 전체**를 말한다.
    ...
    assert wide == {GLOBAL_MAE}, (
        f"§모델 검증 실적이 ±{sorted(wide)}% 라고 한다 — 전체평균 ±{GLOBAL_MAE}% 여야 한다. "
