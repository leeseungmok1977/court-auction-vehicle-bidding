...
## 2-B. 재빌드(앱 업데이트) 절차 — 2026-09-12 추가

**언제 필요한가**: 매니페스트 `name`/`short_name`, 아이콘, 패키지 설정, Play Billing 토글이 바뀔 때.
**웹 콘텐츠만 바뀌면 재빌드 불필요** — TWA라 서버 배포만으로 모든 설치 기기에 즉시 반영된다.

> ⚠️ **v1을 만든 것과 같은 도구(PWABuilder)를 쓴다.** bubblewrap CLI로 직접 빌드할 수도 있으나,
> 기본값이 달라 스플래시·알림 위임 등에서 미묘한 동작 차이가 생길 수 있다.
> 라이브 비공개 테스트가 도는 중에는 도구를 바꾸지 않는 편이 안전하다.

**PWABuilder (All Settings에서 반드시 확인할 값)**
| 항목 | 값 | 함정 |
|---|---|---|
| Package ID | `kr.co.naechaget.app` | 기본값이 `...twa`로 채워진다. **반드시 수정** |
| App name | `경매로 내차GET` | 설정>앱 목록에 표시 |
| Launcher name | `경매로 내차GET` | **홈 화면 아이콘 아래 이름**(매니페스트 `short_name`에서 옴) |
| App version | `1.0.1` | 사람이 읽는 버전 |
| App version code | **2** | 안 올리면 업로드 거부 |
| Signing key | **Mine** + 기존 `signing.keystore` | **New 고르면 업로드 거부**(서명 불일치) |
...
1. 테스트 및 출시 → 테스트 → **비공개 테스트** → **새 버전 만들기**
   - ⚠️ **새 트랙을 만들지 말 것.** 기존 Alpha 트랙에 업데이트로 올려야 테스터 14일 연속이 유지된다.
2. 새 `.aab` 업로드 → 출시 노트 작성 → 저장 후 검토 → 출시 시작
3. **게시 개요 → 변경사항 제출** (이걸 눌러야 심사가 걸린다)
4. 심사 통과 후 테스터 기기에 자동 업데이트
...
> ⚠️ **2026-09-12 실전 정정**: 아래 "앱 서명 키 인증서의 SHA-256을 복사" 지침만으로는 **실패했다.**
> 콘솔이 보여주는 지문 3종(업로드 키 / 앱 서명 키'기존' / 앱 서명 키'양자내성')이 **전부 실제 배포본 서명과 달랐고**,
> 4번째 값 — `이전 앱 서명 키`의 인증서를 내려받아 직접 해시한 `B2:5D:..` — 이라야 검증이 통과했다.
> **확실한 방법은 인증서 파일을 내려받아 직접 SHA-256을 계산하는 것.** 상세 경위는 memory `project-play-store-twa`.

**운영자가 할 일 (콘솔):**
- [ ] `Google Play로 보호됨` → `Play 앱 서명 관리` 열기 (구 "앱 무결성 → 앱 서명"에서 이동됨)
- [ ] **모든 인증서를 ⬇ 다운로드** — `앱 서명 키`(기존·양자내성) + **`이전 앱 서명 키`** + `업로드 키 인증서`
- [ ] 받은 `.der` 파일들의 SHA-256을 직접 계산 (아래 §5)

**계산 방법(확실함, 콘솔 표시값보다 신뢰):**
```python
import hashlib
raw = open(r"C:\...\deployment_cert.der","rb").read()
...
**클로드 코드/서버가 할 일:** ✅ **2026-09-12 완료 — 검증 통과(주소창 사라짐)**
- [x] `web/static/.well-known/assetlinks.json`에 **보유한 인증서 지문을 전부 배열로** 기입
  - Android는 설치본 서명이 **배열 중 하나라도 일치하면 통과** → 다 넣는 게 가장 안전
  - 실제 등록값 4개: `B2:5D:..`(배포 인증서 ★실제 서명) · `01:0D:..`(앱서명 기존) · `39:C0:..`(앱서명 양자내성) · `0A:95:..`(업로드 키, 사이드로드용)
  - `package_name` = `kr.co.naechaget.app` 정확히 유지
- [x] **Google DAL 캐시(maxAge ≈1시간) 갱신 확인 후** 기기에서 *앱 삭제 → 재설치*
  - ⚠️ Android는 검증 결과를 **설치 시점에 고정** — 캐시 갱신 전 재설치하면 실패가 고착된다
  - ⚠️ **DAL API의 PASS는 "파일이 올바르게 게시됨"만 의미**하고, 지문이 실제 서명키와 맞는지는 검증하지 않는다
...
**운영자가 할 일 (콘솔):**
- [x] 신규 개인계정 요건: **테스터 12명 이상 · 14일 연속 비공개 테스트** 통과 후 Production 승격
  - ⚠️ **2026-09-23 콘솔 실측으로 위 가정이 뒤집혔다.** Play Console 대시보드·테스터 탭 화면 기준:

    | | 우리가 적어 뒀던 값 | **콘솔 실제** |
    |---|---|---|
    | 테스터 | 25명 유지(여유 13명) | **초대 34명 / 옵트인 12명 — 여유 0명** |
    | 경과 | 09-12 시작(오늘 11일째) | **"현재 10일"** (시작일 미확인) |
    | 신청 | 09-27 가능 | **'프로덕션 신청' 버튼 비활성** |

    - **12명은 초대 수가 아니라 옵트인 수다**(`CLOSED_TESTING_GUIDE.md:19` 가 이미 경고한 함정).
      34명 목록 `테스터_경매로 내차GET` 중 **22명이 아직 링크를 누르지 않았다.**
    - **여유 0명이 가장 큰 리스크다.** 한 명만 참여를 취소해도 11명이 되어 14일 시계가
      리셋될 수 있다(리셋 조건 자체는 §6 미확인). 손해가 2주라 확률이 낮아도 비대칭이다.
    - 시작일은 **미확인**이다. "10일"이 맞다면 09-14 전후이고, 심사 통과 시점부터 셌을
      가능성이 있다(가이드 §1-④ 각주가 그렇게 될 수 있다고 적어 뒀다).
      → 신청 가능일은 **09-27 또는 09-28**. 확정 전까지 9/27 을 확정일로 쓰지 않는다.
    - ⚠️ **`의견 URL 또는 이메일 주소`가 비어 있다**(0/512). 가이드 §1-② 가 `koreanplus@gmail.com`
      을 넣으라고 지시한 자리다. 테스터가 의견을 보낼 창구가 없는 상태다.
  - 일정 계획에 이 14일을 반드시 포함
  - 👉 **실행 절차·모집 문구·체크리스트는 [CLOSED_TESTING_GUIDE.md](CLOSED_TESTING_GUIDE.md) 참조** (2026-09-12 작성)
  - ⚠️ **내부 테스트는 이 요건에 카운트되지 않는다.** 비공개 테스트 트랙에 별도 출시해야 시계가 돈다(이때 처음으로 앱 심사를 거친다).
...
- **[assetlinks 지문 오류]** 지문이 틀리면 URL바 노출·"그냥 웹사이트"로 반려. ⚠️ **2026-09-12 실전 정정 — "앱 서명 키 SHA-256을 복사"만으로는 실패했다.** 콘솔이 화면에 보여주는 지문(업로드 키·앱 서명 키'기존'·앱 서명 키'양자내성')이 **전부 실제 배포본 서명과 달랐고**, `이전 앱 서명 키`의 인증서(`deployment_cert.der`)를 내려받아 직접 SHA-256을 계산한 값이라야 통과했다. → **인증서를 전부 다운로드해 직접 해시하고, 얻은 지문을 배열에 모두 넣을 것**(§4 참조).
- **[DAL API PASS 오판]** Google Digital Asset Links API가 PASS라고 해도 그건 *"파일이 올바른 형식으로 게시됐다"*는 뜻이지 *"그 지문이 Play가 실제로 서명하는 키와 일치한다"*는 보장이 **아니다**. 최종 판정은 실기기 설치 후 주소창 유무로만 확인.
- **[설치 시점 고정]** Android는 DAL 검증 결과를 **설치 시점에 고정**하고, Google 캐시는 약 1시간(`maxAge`) 유지된다. 파일을 고쳤으면 *캐시 만료 확인 → 앱 삭제 → 재설치* 순서를 반드시 지킬 것.
