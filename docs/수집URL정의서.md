...
- **파라미터**:
  ...
  - `q=` 검색식 (실측 문법):
    ...
    - ⚠️ **분류는 두 단계 — `ModelGroup`(차종) → `Model`(세대)**. 응답 행에는 `Model`만 있고 `ModelGroup` 필드는 없다.
      2026-09-13 승인 조회(14회) 실측:
      - `KG모빌리티(쌍용)` / `렉스턴` (1,766건) 안에 Model `렉스턴 스포츠`·`렉스턴 스포츠 칸`·`G4 렉스턴`·`올 뉴 렉스턴`·`더 뉴 렉스턴 스포츠 (칸)`
        → `렉스턴 스포츠`·`G4 렉스턴`을 ModelGroup 자리에 넣으면 **0건**. 픽업/SUV는 `Model` 이름으로 걸러야 한다(`encar.model_hint`).
      - `현대` / `제네시스` (1,017건, Model `제네시스 DH`) — 2008~2016 제네시스는 브랜드 `제네시스`가 아니라 `현대` 밑.
      - `미니` / `쿠퍼` (Model 쿠퍼·쿠퍼 S·쿠퍼 D) · `벤츠` / `CLS-클래스` (Model `CLS-클래스 W218`) · `BMW` / `M4` (Model `M4 (G82)`)
      - 수입 ModelGroup은 한글 음차가 원칙: `포드`/`익스플로러`, `폭스바겐`/`파사트`, `렉서스`/`RX`(Model `RX350h 5세대`) — 단 `도요타`/`RAV4`는 영문이 정식(`라브4`는 0건).
      - `마세라티` 제조사만 조회 시 Model `르반떼`·`기블리`·`콰트로포르테`. `기아`/`레이` 안에 Model `더 뉴 기아 레이 EV`(FuelType 전기).
      - 재조회 결과로 추가 확인(2026-09-13 프로덕션): `미니`/`컨트리맨`(Model `쿠퍼 컨트리맨`)·`미니`/`클럽맨`(`쿠퍼 클럽맨`)·`푸조`/`3008`(`3008 2세대`)·
        `쉐보레(GM대우)`/`콜로라도` **국산(Y)** (Model `콜로라도`). 미확인: `이쿼녹스`(Y·N 모두 0건 — 2019 표본 부족일 수도), `트래버스`, `티구안`·`코세어` 별칭.
